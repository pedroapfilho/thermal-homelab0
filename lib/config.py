...
class ConfigHandler:
    ...
    def load() -> Config:
        ...
        config = Config(
            type=os.getenv('THERMAL_TYPE', 'network').strip().lower(),
            ip=os.getenv('THERMAL_IP', '127.0.0.1').strip(),
            port=int(os.getenv('THERMAL_PORT', '9100').strip()),
            max_lines=int(os.getenv('THERMAL_MAX_LINES', '30').strip()),
            line_width=int(os.getenv('THERMAL_LINE_WIDTH', '48').strip()),
            vendor_id=int(os.getenv('THERMAL_VENDOR_ID', '0x04b8').strip(), 0),
            product_id=int(os.getenv('THERMAL_PRODUCT_ID', '0x0e20').strip(), 0),
        )
